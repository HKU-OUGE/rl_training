...
def main():
    ...
    # per-axis breakdown (y-velocity learning diagnosis)
    cum_lin_err_x = torch.zeros(args.num_envs, device=device)
    cum_lin_err_y = torch.zeros(args.num_envs, device=device)
    cum_speed_x = torch.zeros(args.num_envs, device=device)
    cum_speed_y = torch.zeros(args.num_envs, device=device)
    cum_cmd_x_mag = torch.zeros(args.num_envs, device=device)
    cum_cmd_y_mag = torch.zeros(args.num_envs, device=device)
    ...
    for step in range(args.num_steps):
        ...
        # per-axis (y-velocity diagnosis)
        cum_lin_err_x += (cmd[:, 0] - v_b[:, 0]).abs()
        cum_lin_err_y += (cmd[:, 1] - v_b[:, 1]).abs()
        cum_speed_x += v_b[:, 0].abs()
        cum_speed_y += v_b[:, 1].abs()
        cum_cmd_x_mag += cmd[:, 0].abs()
        cum_cmd_y_mag += cmd[:, 1].abs()
        ...
        if dones.any():
            ...
            for i, env_id in enumerate(done_idx.tolist()):
                ...
                episodes["err_x"].append(float((cum_lin_err_x[env_id] / max(ep_len, 1)).item()))
                episodes["err_y"].append(float((cum_lin_err_y[env_id] / max(ep_len, 1)).item()))
                episodes["spd_x"].append(float((cum_speed_x[env_id] / max(ep_len, 1)).item()))
                episodes["spd_y"].append(float((cum_speed_y[env_id] / max(ep_len, 1)).item()))
                episodes["cmd_x_mag"].append(float((cum_cmd_x_mag[env_id] / max(ep_len, 1)).item()))
                episodes["cmd_y_mag"].append(float((cum_cmd_y_mag[env_id] / max(ep_len, 1)).item()))
            ...
            cum_lin_err_x[done_idx] = 0
            cum_lin_err_y[done_idx] = 0
            cum_speed_x[done_idx] = 0
            cum_speed_y[done_idx] = 0
            cum_cmd_x_mag[done_idx] = 0
            cum_cmd_y_mag[done_idx] = 0
    ...
    # per-axis arrays
    ep_err_x = np.array(episodes["err_x"])
    ep_err_y = np.array(episodes["err_y"])
    ep_spd_x = np.array(episodes["spd_x"])
    ep_spd_y = np.array(episodes["spd_y"])
    ep_cmd_x = np.array(episodes["cmd_x_mag"])
    ep_cmd_y = np.array(episodes["cmd_y_mag"])
    ...
    overall = {
        ...
        # per-axis (y-velocity diagnosis)
        "mean_err_x": float(ep_err_x.mean()) if ep_err_x.size else 0.0,
        "mean_err_y": float(ep_err_y.mean()) if ep_err_y.size else 0.0,
        "mean_speed_x": float(ep_spd_x.mean()) if ep_spd_x.size else 0.0,
        "mean_speed_y": float(ep_spd_y.mean()) if ep_spd_y.size else 0.0,
        "mean_cmd_x_mag": float(ep_cmd_x.mean()) if ep_cmd_x.size else 0.0,
        "mean_cmd_y_mag": float(ep_cmd_y.mean()) if ep_cmd_y.size else 0.0,
    }
    ...
    for t_idx, t_name in enumerate(terrain_type_names):
        ...
        per_type[t_name] = {
            ...
            # per-axis breakdown
            "err_x": float(ep_err_x[m].mean()),
            "err_y": float(ep_err_y[m].mean()),
            "spd_x": float(ep_spd_x[m].mean()),
            "spd_y": float(ep_spd_y[m].mean()),
            "cmd_x_mag": float(ep_cmd_x[m].mean()),
            "cmd_y_mag": float(ep_cmd_y[m].mean()),
            "reward_per_step": {
    ...
    # per-axis (y-velocity diagnosis)
    print(f"  per-axis cmd:   |vx_cmd|={overall['mean_cmd_x_mag']:.3f}  |vy_cmd|={overall['mean_cmd_y_mag']:.3f}")
    print(f"  per-axis vel:   |vx|    ={overall['mean_speed_x']:.3f}  |vy|    ={overall['mean_speed_y']:.3f}")
    print(f"  per-axis err:   err_x   ={overall['mean_err_x']:.3f}  err_y   ={overall['mean_err_y']:.3f}")
    if overall['mean_cmd_y_mag'] > 0.01:
        _y_track = 1.0 - overall['mean_err_y'] / max(overall['mean_cmd_y_mag'], 1e-6)
        print(f"  y-track-ratio:  1 - err_y/|vy_cmd| = {_y_track:.2%}  "
              f"(>50% = decent, ≈0% = ignored)")

    print("\n  Per-terrain-type:")
    print(f"    {'type':<18} {'n':>5} {'len':>6} {'illegal':>8} {'lin_err':>8} {'spd':>6} {'lvl':>6}")
    for t_name in terrain_type_names:
        s = per_type.get(t_name)
        if s is None:
            continue
        print(f"    {t_name:<18} {s['n_eps']:>5} {s['mean_len']:>6.1f} "
              f"{s['illegal_pct']:>7.1%} {s['lin_err']:>8.3f} "
              f"{s['mean_speed']:>6.3f} {s['mean_level']:>6.1f}")

    print("\n  Per-terrain y-velocity tracking:")
    print(f"    {'type':<18} {'|vy_cmd|':>9} {'|vy|':>7} {'err_y':>7} {'y-ratio':>8}")
    for t_name in terrain_type_names:
        s = per_type.get(t_name)
        if s is None:
            continue
        y_ratio = 1.0 - s['err_y'] / max(s['cmd_y_mag'], 1e-6) if s['cmd_y_mag'] > 0.01 else float('nan')
        ratio_str = f"{y_ratio:7.1%}" if not np.isnan(y_ratio) else "    n/a"
        print(f"    {t_name:<18} {s['cmd_y_mag']:>9.3f} {s['spd_y']:>7.3f} "
              f"{s['err_y']:>7.3f} {ratio_str}")
