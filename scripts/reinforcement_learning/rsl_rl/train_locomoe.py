"""Trainer for the LocoMoE baseline (MoE-Loco, Huang et al., IROS 2025).

Mirror of ``train_moe.py`` with three differences only:
  1. Imports ``LocoMoEActorCritic`` / ``LocoMoEPPO`` from ``locomoe_terrain``.
  2. Injects those classes into ``rsl_rl.modules`` / ``rsl_rl.runners.on_policy_runner``
     so the runner's ``eval(class_name)`` call finds them.
  3. Default ``--task`` points at ``Rough-LocoMoE-Teacher-Deeprobotics-M20-v0``.

Everything else (PER_RANK_TERRAIN dispatch, distributed/wandb logic, video
recording, checkpoint resume) is preserved as-is for parity with SplitMoE.
"""
...
parser = argparse.ArgumentParser(description="Train LocoMoE Policy (baseline, head-to-head vs SplitMoE)")
parser.add_argument("--task", type=str, default="Rough-LocoMoE-Teacher-Deeprobotics-M20-v0", help="Task name")
...
try:
    ...
    print("[Info] Imported LocoMoE classes from current directory.")
except ImportError:
    try:
        from rl_training.tasks.manager_based.locomotion.velocity.config.wheeled.deeprobotics_m20.agents.locomoe_terrain import (
            LocoMoEActorCritic,
        ...
        print("[Info] Imported LocoMoE classes from project path.")
    except ImportError:
        raise ImportError(
            "Could not import LocoMoEActorCritic/LocoMoEPPO/LocoMoEPPOCfg from locomoe_terrain.py"
        )

# === 注入到 RSL-RL ===
# rsl_rl's OnPolicyRunner constructs the policy via ``eval(class_name)``, so the
# class strings in LocoMoEPPOCfg must resolve in the runner's namespace.
...
def main():
    ...
    # Honor the cfg's policy class_name (set by task registry). For LocoMoE
    # task this resolves to "LocoMoEActorCritic" (declared in LocoMoEActorCriticCfg).
    policy_class = train_cfg_dict["policy"].get("class_name", "LocoMoEActorCritic")
